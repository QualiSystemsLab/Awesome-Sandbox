    ...
    def push_data_from_pool_to_sandbox(self):
        ...
        for resource_from_pool in self.pool_resource.details.ChildResources:
            ...
            for sandbox_resource in  sandbox_resources:
                #use a regular expression since the pool may contain resources with a running index at the end of the name ,
                #that should match a generic alias for multiple resources in the sandbox.
                ...
                if found_pattern:
                    if sandbox_resources_can_update[idx]:
                        ...
                        for attribute in resource_from_pool.ResourceAttributes:
                            sandbox_resource.set_attribute_value(attribute_name=attribute.Name,attribute_value=attribute.Value)
                        break
                idx = idx+1
